...
<template>
  <div class="form-container bg-white min-h-screen flex flex-col items-center pt-10">
    <!-- Прогресс-бар -->
    <div class="w-full max-w-3xl h-3 rounded-full bg-gray-200 mb-8 shadow-sm overflow-hidden sticky top-0 z-20">
      <div
        class="h-full bg-gradient-to-r from-purple-500 to-indigo-400 transition-all duration-500"
        :style="{ width: progress + '%' }"
      ></div>
    </div>
    <div class="form-content w-full max-w-3xl">
      <Transition name="fade" mode="out-in">
        <UCard class="bg-white shadow-xl rounded-2xl border border-gray-300">
          <template #header>
          ...
          <template #default>
            <div class="flex flex-col gap-4">
              <template v-if="currentQuestion?.text">
                <p class="whitespace-pre-line text-gray-800">{{ currentQuestion.text }}</p>
                <UDivider v-if="currentQuestion.hint || currentQuestion.example" />
              </template>
              <p v-if="currentQuestion?.hint" class="text-sm text-gray-500">{{ currentQuestion.hint }}</p>
              <p v-if="currentQuestion?.example" class="text-sm italic text-gray-400">{{ currentQuestion.example }}</p>
              <!-- Question Inputs -->
              <template v-if="currentStep !== 0">
                <textarea
                  ...
                  :rows="5"
                  class="w-full light:bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-300 text-gray-900 p-3 resize-y min-h-[120px]"
                />
                <UInput
                  v-else-if="currentQuestion?.type === 'input' && currentQuestion.field"
                  v-model="formData[currentQuestion.field as keyof FormData]"
                  class="w-full bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-300 text-gray-900"
                ...
                <div v-else-if="currentQuestion?.type === 'radio' && currentQuestion.field" class="space-y-3">
                  <label 
                    ...
                    :class="{ 'bg-purple-50 border-purple-300': formData[currentQuestion.field as keyof FormData] === option.value }"
                  >
                    <input
                      type="radio"
                      :value="option.value"
              ...
              <div v-else-if="currentQuestion?.type === 'final'" class="space-y-4">
                <div class="text-center">
                  <p class="text-lg text-gray-700 mb-6">Все ответы сохранены! Готов получить свой результат?</p>
                  <UButton 
                    color="primary" 
                    class="bg-gradient-to-r from-purple-500 to-indigo-400 text-white shadow-md h-14 text-xl px-12" 
              ...
              <div v-if="currentQuestion?.type !== 'final'" class="flex justify-between gap-2 mt-6">
                <UButton
                  ...
                  @click="prevStep"
                  class="min-w-[120px] bg-gray-100 text-gray-700 border border-gray-300 h-14 text-lg px-8"
                >Назад</UButton>
                <div v-else></div>
                <UButton
                  ...
                  @click="nextStep"
                  class="min-w-[120px] bg-gradient-to-r from-purple-500 to-indigo-400 text-white shadow-md h-14 text-lg px-8"
                >Далее</UButton>
              </div>
            </div>
          </template>
